  ...
  function setHeaderHeight() {
    ...
    if (header) {
      document.documentElement.style.setProperty('--header-height', `${parseInt(header.getBoundingClientRect().bottom)}px`);
    }
  }
  ...
  class MenuDrawer extends HTMLElement {
    ...
    openMenuDrawer(summaryElement) {
      ...
      setTimeout(() => {
        if ( this.classes.open.includes('mini-cart') ){
          const minicartForm = document.getElementById('cart');
          const checkoutStarted = () => {
            const minicartFormOnSubmit = document.getElementById('cart');
            const onsubmitJson = JSON.parse(minicartFormOnSubmit.getAttribute('data-ga4-onsubmit'));
            dataLayer.push(onsubmitJson)
          }
          minicartForm.addEventListener('submit', checkoutStarted);
        }
      }, 1000)
    ...
    updateGangMedia() {
      ...
      if (this.currentGangValue !== gangValue) {
        ...
        const newMedias = document.querySelectorAll(
          `#shopify-section-${this.dataset.section} [data-gang-connect="${gangConnect}"]`
        );
        ...
        if (gallerySliderButtons) {
          if (document.querySelectorAll(`#shopify-section-${this.dataset.section} product-gallery .product__media-item.gang__active`).length < 2) {
            gallerySliderButtons.classList.add('hidden');
          } else {
            gallerySliderButtons.classList.remove('hidden');
          }
      ...
      if (!this.currentVariant.featured_media) {
        if(!document.querySelectorAll(`#shopify-section-${this.dataset.section} product-gallery .product__media-item.gang__active.is-active`).length) {
          document.querySelector(`#shopify-section-${this.dataset.section} product-gallery .product__media-item.gang__active`).classList.add('is-active');
        }
        return;
      }
    ...
    toggleAddButton(disable = true, text, modifyClass = true) {
      ...
      productForms.forEach((productForm) => {
        ...
        if (disable) {
          ...
          addButton.textContent = addButton.dataset.preOrder === 'true' ? variantStrings.preOrder : variantStrings.addToCart;
        }
    ...
    onSubmitHandler(evt) {
      ...
      fetch(`${routes.cart_add_url}`, config)
        ...
        .then((response) => {
          ...
          if (dataLayer) {  // Clear the previous ecommerce object.
              dataLayer.push({
                'event': 'addToCart',
                'ecommerce': {
                  'currencyCode': 'USD',
  ...
  class PriceMoney extends HTMLElement {
    ...
    shouldInit() {
      const moneyFormat = shopSettings.moneyFormat.toLowerCase();
    ...
    init() {
      ...
      const symbol = shopSettings.moneyFormat.replace(/\{{.*}}/, '').trim();
  ...
  class CountdownTimer extends HTMLElement {
    ...
    timer() {
      ...
      if (this.dataset.compact == 'true') {
        ...
        const hourHTML = `<div class="countdown__item"><span>${hours}</span> ${hours == 1 ? dateStrings.hour : dateStrings.hours}</div>`;
        const minHTML = `<div class="countdown__item"><span>${mins}</span> ${mins == 1 ? dateStrings.minute : dateStrings.minutes}</div>`;
        const secHTML = `<div class="countdown__item"><span>${secs}</span> ${secs == 1 ? dateStrings.second : dateStrings.seconds}</div>`;
  ...
  class BundleProduct extends HTMLElement {
    ...
    onSelectChange(event) {
      ...
      price__sale.querySelector('.price-item--regular').innerHTML = `<price-money><bdi>${theme.Currency.formatMoney(compare_at_price, shopSettings.moneyFormat)}</bdi></price-money>`;
      price__sale.querySelector('.price-item--sale').innerHTML = `<price-money><bdi>${theme.Currency.formatMoney(price, shopSettings.moneyFormat)}</bdi></price-money>`;
    }
  }
  ...
  class ShopTheLook extends HTMLElement {
    ...
    init() {
      ...
      this.products.querySelectorAll('.quick-view__summary').forEach((button) => button.addEventListener('click', this.onButtonClick.bind(this)));
    ...
    onButtonClick(event) {
      ...
      const button = this.querySelector(`lookbook-component [data-block-index="${block.dataset.blockIndex}"]`);
      const summaryElement = button.closest('summary');
  ...
  class CartSampleProducts extends HTMLElement {
    constructor() {
      ...
      const sampleProductsContainer = this.querySelector('.cart-sample-products');
      const products = this.querySelectorAll('.cart-sample-product');
      const sampleProducts = Array.from(this.querySelectorAll('div[data-product-id]')).map((product) => parseInt(product.dataset.productId));
      ...
      function slide(direction) {
        let left;
        const { scrollLeft, clientWidth } = sampleProductsContainer;
        switch (direction) {
          case "prev":
        ...
        sampleProductsContainer.scroll({
          left,
          behavior: "smooth"
        });
      }
  
      if (sampleProductsContainer && prevButton && nextButton) {
        prevButton.addEventListener("click", () => slide("prev"));
        nextButton.addEventListener("click", () => slide("next"));
      }
  
      if (sampleProductsContainer.scrollLeft === 0) {
        prevButton.setAttribute("disabled", true);
      }
  
      sampleProductsContainer.addEventListener("scroll", () => {
        //if last slide hide next button
        if (sampleProductsContainer.scrollWidth - sampleProductsContainer.scrollLeft === sampleProductsContainer.clientWidth) {
      ...
      this.checkforSamples(sampleProducts,products);
    }
    checkforSamples(sampleProducts,products){
      let itemtoremove = [];
      const maxSamples = 2;
      let noSamples = 0;
      let totalItems = 0;
      let miniCart = document.querySelector('mini-cart');
          fetch(`/cart.js`)
            ...
            .then(data => {
              ...
              const sampleInCart = cartItems.some(item => sampleProducts.includes(item.id));
              if (sampleInCart) {
                products.forEach((product) => {
                  product.classList.remove('deactive');
                  product.classList.remove('active');
                  product.querySelector('input[type="checkbox"]').removeAttribute('disabled');
                });
              } else {
                products.forEach((product) => {
                  product.classList.remove('deactive');
                  product.classList.remove('active');
                  product.querySelector('input[type="checkbox"]').removeAttribute('disabled');
                  product.querySelector('input[type="checkbox"]').checked = false;
                });
              ...
              cartItems.forEach((item,index) => {
                if (sampleProducts.includes(item.id)) {
                  ...
                  if(noSamples == 2){
                    ...
                    const sample_Products = document.querySelectorAll('div[data-product-id]');
                    sample_Products.forEach(smplProduct => {
                      if (!smplProduct.classList.contains('active')) {
                        smplProduct.classList.add('deactive');
                      }
                    });
                  }
                  // if(noSamples > 2){
                  //   if (noSamples > maxSamples) {
                  //         //document.querySelector('[data-variant-id="'+cartItem.id+'"] cart-remove-button').click();
                  //         let itemupdate = document.querySelector('[data-variant-id="'+item.id+'"]');
                  //         const itemIndex = document.querySelector('[data-variant-id="'+item.id+'"] cart-remove-button').getAttribute('data-index');
                  //         itemupdate.closest('cart-items').updateQuantity(itemIndex, 0);
                  //         //this.updateSample(cartItem.key,0);
                  //         return;
                  //       }
                  // }
                }
              ...
              for(let i=0; i< totalItems; i++){
                let item = cartItems[i];
                    if (sampleProducts.includes(item.id)) {
                        let itemupdate = document.querySelector('[data-variant-id="'+item.id+'"]');
                        const sampleProduct = document.querySelector(`div[data-product-id="${item.id}"]`);
                        sampleProduct.classList.add('active');
                        sampleProduct.classList.remove('deactive');
                        sampleProduct.querySelector('input[type="checkbox"]').checked = true;
                        console.log(item.product_title);                
                        const checkbox = sampleProduct.querySelector('input[type="checkbox"]');
                        checkbox.setAttribute('disabled', true);
                        checkbox.checked = true;
                          if(item.quantity > 1) {
                            const itemIndex = document.querySelector('[data-variant-id="'+item.id+'"] cart-remove-button').getAttribute('data-index');
                            itemupdate.closest('cart-items').updateQuantity(itemIndex, 1);
                            //this.updateSample(item.key,1)
                            return false;
                          }else if (noSamples > maxSamples) {
                            itemtoremove.push(item.key);
                          }
                    }
               }
